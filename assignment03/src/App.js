...
function App() {
  ...
  const editProduct = async (product_id, new_price) => {
    ...
    await fetch(
      "http://localhost:8081/editProduct/" + product_id,
      requestOptions
    );
  };
  ...
  const deleteProduct = async (product_id) => {
    ...
    await fetch(
      "http://localhost:8081/deleteProduct/" + product_id,
      requestOptions
    );
  };
  ...
  const getCardForProduct = (el) => {
    let card = (
      ...
      >
        ...
        />
        <Card.Body
          className="d-flex flex-column card-content"
          style={{ height: "100%" }}
        >
          <Card.Title className="card-title" style={{ height: "20%" }}>
            {el.title}
          </Card.Title>
          <Card.Text className="card-text" style={{ height: "5%" }}>
            Product Id: {el.id}
          </Card.Text>
          <Card.Text className="card-text" style={{ height: "5%" }}>
            Price: {numToPrice(el.price)}
          </Card.Text>
          <Card.Text className="card-text" style={{ height: "50%" }}>
            Description: {el.description}
          </Card.Text>
          <Card.Text className="card-text" style={{ height: "10%" }}>
            Product Rating: {el.rating.rate}
          </Card.Text>
          <Card.Text className="card-text" style={{ height: "10%" }}>
  ...
  function Add() {
    ...
    const form = (
      <form onSubmit={handleSubmit(onSubmit)} className="container mt-5">
        ...
        <div className="form-group">
          <input
            {...register("ratingScore", { required: true })}
              ...
              <div className="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                ...
                <div className="col-auto d-none d-lg-block">
                  <img
                    width="200"
                    height="250"
                    src="./images/Kearns.jpg"
                    aria-label="Drew Kearns"
                    preserveAspectRatio="xMidYMid slice"
                    focusable="false"
                  />
                  <title>Drew Kearns</title>
                </div>
              </div>
            </div>
            <div className="col-md-6">
              <div className="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                ...
                <div className="col-auto d-none d-lg-block">
                  <img
                    ...
                    height="250"
                    src="./images/Kyle.jpg"
                    aria-label="Kyle Maloney"
                    preserveAspectRatio="xMidYMid slice"
                    focusable="false"
                  />
                  <title>Kyle Maloney</title>
                </div>
              </div>
            </div>
          </div>
